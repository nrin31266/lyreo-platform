...
latency, availability hay model accuracy khi chưa có quyết định; xem [OQ-003](open-questions.md#oq-003--nfr-targets-and-success-metrics).

### NFR-SEC-001 — Authorization phía server

Core phải enforce role/resource ownership; ẩn menu phía client không đủ. Learner
...
Provider keys phải mã hóa AES-GCM bằng master key server-side; API read chỉ trả
...
Speech recording và learner artifacts mặc định private, cấp quyền qua access có
thời hạn; database chỉ lưu object key.

### NFR-OPS-001 — Durable and recoverable jobs

Job phải sống qua restart, có cancel/retry/lease/heartbeat, resume idempotent step
...
Request/job/AI invocation phải có identifiers liên kết được; failure giữ stable
...
Expensive/outbound calls cần timeout, cancellation check và policy
...
Import ghi dataset/version/checksum/status/count/error, hỗ trợ validate/dry-run và
rerun có kiểm soát. Source/license attribution phải được giữ; dataset lớn không commit Git.

### NFR-DAT-002 — Schema authority

Flyway là schema owner; Hibernate chỉ validate. Mỗi schema change có migration
mới và cần startup/validation evidence phù hợp.

### NFR-UI-001 — Accessible resilient UI states

Feature UI phải xử lý loading/error/empty và network failure, dùng semantic theme
...
Mỗi invocation lưu capability, actual provider/model, status, timing/cost metadata
...
Speech, raw AI, chat, temp upload và cached media cần retention policy/maintenance
